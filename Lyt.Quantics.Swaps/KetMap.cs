...
public sealed class KetMap
{
    ...
    public KetMap(int qubitCount)
    {
        ...
        this.map = new List<List<bool>>(this.length);
        ...
        this.FastMapBits = new bool[this.length][];
        for (int i = 0; i < this.length; i++)
        {
            this.FastMapBits[i] = [.. this.map[i]]; 
        }

        this.FastMapValues = new int[this.length];
        for (int i = 0; i < this.length; i++)
        {
            int sum = 0;
            int bitValue = 1;
    ...
    {
        if ((ket1 < 0) || (ket1 >= this.qubitCount))
        {
            throw new ArgumentException("Invalid ket index: ket1");
        }

        if ((ket2 < 0) || (ket2 >= this.qubitCount))
        {
        ...
        int min = Math.Min(ket1, ket2);
        int max = Math.Max(ket1, ket2);
        ...
        reducedKetMap.FastMapBits = new bool[this.length][];
        for (int i = 0; i < this.length; i++)
        {
            reducedKetMap.FastMapBits[i] = [.. reducedKetMap.map[i]];
        }

        reducedKetMap.FastMapValues = new int[this.length];
        for (int i = 0; i < this.length; i++)
        {
            int sum = 0;
            int bitValue = 1;
...
#endif // DEBUG
        bool[][] reducedFastMapBits = reducedKetMap.FastMapBits;
